...
class OneFoldTrainer:
    def __init__(self, args, fold, config):
        ...
        if self.tp_cfg['mode'] == 'SCL':
            ...
            self.ckpt_path = os.path.join('checkpoints_pr',self.cfg['dataset']['name'], self.tp_cfg['mode'], str(self.tp_cfg['tau']) )
            ...
            self.ckpt_path = os.path.join('checkpoints_pr',self.cfg['dataset']['name'], self.tp_cfg['mode'], str(self.tp_cfg['alpha']), str(self.tp_cfg['tau']) )
            ...
            self.criterion = PCL()
            self.ckpt_path = os.path.join('checkpoints_pr',self.cfg['dataset']['name'], self.cfg['backbone']['name'],self.tp_cfg['mode'], str(self.tp_cfg['alpha']), str(self.tp_cfg['tau']) )

        elif self.tp_cfg['mode'] == 'Ours_WOH':
            self.criterion = Ours_WOH(config= self.cfg, device = self.device)
            self.ckpt_path = os.path.join('checkpoints_pr',self.cfg['dataset']['name'], self.tp_cfg['mode'], str(self.tp_cfg['alpha']), str(self.tp_cfg['tau']) )
        
        elif self.tp_cfg['mode'] == 'Ablation':
            self.criterion = Ablation(config= self.cfg, device = self.device)
            self.ckpt_path = os.path.join('checkpoints_pr',self.cfg['dataset']['name'], self.tp_cfg['mode'], str(self.tp_cfg['alpha']), str(self.tp_cfg['tau']),  str(self.tp_cfg['Lambda']))
