...
class OneFoldTrainer:
    def __init__(self, args, fold, config):
        ...
        if self.tp_cfg['mode'] == 'SCL':
            self.ckpt_path = os.path.join('checkpoints_ft_fp',self.cfg['dataset']['name'], self.tp_cfg['mode'], str(self.tp_cfg['tau']))
            ...
            self.ckpt_path = os.path.join('checkpoints_ft',self.cfg['dataset']['name'], self.tp_cfg['mode'], str(self.tp_cfg['h']))
            self.ckpt_path_from = os.path.join('checkpoints_pr2',self.cfg['dataset']['name'], self.tp_cfg['mode'], str(self.tp_cfg['h']))

        else:
            if self.tp_cfg['mode'] == 'Ablation':
                self.ckpt_path_from = os.path.join('checkpoints_pr',self.cfg['dataset']['name'], self.tp_cfg['mode'], str(self.tp_cfg['alpha']), str(self.tp_cfg['tau']), str(self.tp_cfg['Lambda']))
                self.ckpt_path = os.path.join('checkpoints_ft',self.cfg['dataset']['name'], self.tp_cfg['mode'], str(self.tp_cfg['alpha']), str(self.tp_cfg['tau']), str(self.tp_cfg['Lambda']))

            else:
                self.ckpt_path_from = os.path.join('checkpoints_pr',self.cfg['dataset']['name'], self.tp_cfg['mode'], str(self.tp_cfg['alpha']), str(self.tp_cfg['tau']))
                self.ckpt_path = os.path.join('checkpoints_ft',self.cfg['dataset']['name'], self.tp_cfg['mode'], str(self.tp_cfg['alpha']), str(self.tp_cfg['tau']))


            # self.ckpt_path_from = os.path.join('checkpoints_pr',self.cfg['dataset']['name'], self.tp_cfg['mode'],  str(self.tp_cfg['alpha']), str(self.cfg['dataset']['split_len'] ))
    ...
    #     Pretrain_model = Base_hierarchical(self.cfg)
    #     Pretrain_model = torch.nn.DataParallel(Pretrain_model, device_ids=list(range(len(self.args.gpu.split(",")))))
    ...
    #     model = simpleMLP(self.cfg)

    #     model = torch.nn.DataParallel(model, device_ids=list(range(len(self.args.gpu.split(",")))))
    #     model.to(self.device).to(self.device)
    ...
    def train_one_epoch(self):
        ...
        for i, (inputs, labels,subject_lables) in enumerate(self.loader_dict['train']):
            ...
            outputs = self.model(inputs)
            loss = sum(self.criterion(output, labels) for output in outputs)
            ...
            if self.train_iter % self.tp_cfg['val_period'] == 0:
                print('')
                *_, val_loss = self.evaluate(mode='val')
    ...
    def evaluate(self, mode):
        ...
        return y_true, y_pred, val_acc, eval_loss
        ...
        # elif mode == 'test':
        #     return y_true, y_pred, val_acc
        # elif mode == 'test':
        #     correct, total, eval_loss = 0, 0, 0
        #     for i, (inputs, labels, subject_lables) in enumerate(self.loader_dict['val']):
        #         loss = 0
        #         total += labels.size(0)
        #         inputs = inputs.to(self.device)
        #         labels = labels.view(-1).to(self.device)
        ...
        #         progress_bar(i, len(self.loader_dict['val']), 'Loss: %.3f | Acc: %.3f%% (%d/%d)'
        #             % (eval_loss / (i + 1), 100. * correct / total, correct, total))
    ...
    def run(self):
        ...
        y_true, y_pred,_,_ = self.evaluate(mode='test')
        print('')
...
def main():
    ...
    for fold in [31,30,29,28,27,26,25,24,23,22,21]:
        ...
        accuracy, macro_f1, kappa = summarize_result(config, fold, Y_true, Y_pred)
